...
# Função para criar as tabelas no banco de dados xman
def criar_tabelas():
    criar_db()
    conn = conectar_bd()
    cursor = conn.cursor()
...
def popular_tabelas():
    ...
    try:
        # Inserir dados na tabela Aluno (representando os personagens dos X-Men)
        cursor.execute("""
            INSERT INTO Aluno (nome, idade, habilidades, nivel_poder, equipe, status_matricula)
            VALUES
                ('Scott Summers', 35, 'Laser óptico', 'Alto', 'X-Men', 'Ativo'),
                ('Jean Grey', 32, 'Telepatia, Telecinese', 'Alto', 'X-Men', 'Ativo'),
                ('Logan', 150, 'Fator de cura, Garras de adamantium', 'Alto', 'X-Men', 'Ativo'),
                ('Ororo Munroe', 30, 'Controle do clima', 'Alto', 'X-Men', 'Ativo'),
                ('Charles Xavier', 60, 'Telepatia, Telecinese', 'Alto', 'X-Men', 'Ativo')
        """)

        # Inserir dados na tabela Aula (representando diferentes tipos de treinamentos)
        cursor.execute("""
            INSERT INTO Aula (nome, instrutor, vagas)
            VALUES
                ('Treinamento de Controle de Poderes', 'Prof. Xavier', 20),
                ('Defesa Psíquica', 'Prof. Jean Grey', 15),
                ('Treinamento de Combate', 'Prof. Logan', 25)
        """)

        # Inserir dados na tabela Missao (representando missões dos X-Men)
        cursor.execute("""
            INSERT INTO Missao (objetivo, equipe_designada, data_inicio, data_termino, status)
            VALUES
                ('Salvar mutantes em perigo', 'X-Men', '2024-07-01', '2024-07-10', 'Em andamento'),
                ('Enfrentar a Irmandade de Mutantes', 'X-Men', '2024-07-05', '2024-07-15', 'Planejada'),
                ('Investigar atividades suspeitas da SHIELD', 'X-Men', '2024-06-25', '2024-07-05', 'Concluída')
        """)

        # Inserir dados na tabela Equipe (representando os X-Men como equipe)
        cursor.execute("""
            INSERT INTO Equipe (nome, instrutor)
            VALUES
                ('X-Men', 'Prof. Charles Xavier')
        """)

        # Inserir dados na tabela membros_equipe (associando os X-Men à equipe X-Men)
        cursor.execute("""
            INSERT INTO membros_equipe (equipe_id, aluno_id)
            VALUES
                (1, 1),  -- X-Men: Scott Summers (Ciclope)
                (1, 2),  -- X-Men: Jean Grey
                (1, 3),  -- X-Men: Logan (Wolverine)
                (1, 4),  -- X-Men: Ororo Munroe (Tempestade)
                (1, 5)   -- X-Men: Charles Xavier (Professor X)
        """)
        ...
        print(f"Erro ao popular tabelas: {err}")

    finally:
        cursor.close()
        conn.close()

# Chamar a função para criar as tabelas no banco de dados
criar_tabelas()

# Chamar a função para popular as tabelas com dados fictícios dos X-Men
popular_tabelas()
...
class Equipe:
    ...
    def carregar_bd():
        ...
        try:
            cursor.execute("""
                SELECT e.id, e.nome, e.instrutor, m.aluno_id
                FROM Equipe e
            ...
            for row in cursor:
                ...
                # Aqui você pode adicionar membros à lista 'membros' da equipe
                equipes.append(equipe)

            return equipes

        except mysql.connector.Error as err:
            print(f"Erro ao carregar equipes do banco de dados: {err}")
            return []
...
def buscar_aluno_por_nome(nome):
    ...
    if resultado:
        aluno = {
            ...
            'nivel_poder': resultado[4],
            'equipe': resultado[5],
            'status_matricula': resultado[6]
        }
        return aluno
    else:
        return None
...
class Sistema:
    ...
    def buscar_aluno_por_nome(self, nome):
        return buscar_aluno_por_nome(nome)
    ...
    def consultar_equipes(self):
        ...
        result = []
        for equipe in equipes:
            cursor.execute("SELECT Aluno.nome FROM membros_equipe JOIN Aluno ON membros_equipe.aluno_id = Aluno.id WHERE membros_equipe.equipe_id = %s", (equipe[0],))
            membros = cursor.fetchall()
            membros_nomes = [membro[0] for membro in membros]
            result.append({
                'id': equipe[0],
                'nome': equipe[1],
                'instrutor': equipe[2],
    ...
    # Função para registrar uma missão no banco de dados
    def registrar_missao(objetivo, equipe_designada, data_inicio, data_termino, status):
        conn = conectar_bd()
        cursor = conn.cursor()
...
def registrar_missao_interface():
    objetivo = entry_objetivo_missao.get()
    equipe_designada = entry_equipe_missao.get()
    data_inicio = entry_data_inicio_missao.get()
    data_termino = entry_data_termino_missao.get()
    status = entry_status_missao.get()

    if objetivo and equipe_designada and data_inicio and data_termino and status:
        try:
            # Validar formato da data
            data_inicio = datetime.strptime(data_inicio, '%Y-%m-%d').date()
            data_termino = datetime.strptime(data_termino, '%Y-%m-%d').date()
            
            cadastrar_missao(objetivo, equipe_designada, data_inicio, data_termino, status)
            messagebox.showinfo("Sucesso", "Missão registrada com sucesso!")
        except ValueError:
...
ttk.Label(tab8, text="Objetivo da Missão:").grid(column=0, row=0, padx=10, pady=5)
entry_objetivo_missao = ttk.Entry(tab8)
entry_objetivo_missao.grid(column=1, row=0, padx=10, pady=5)

ttk.Label(tab8, text="Equipe Designada:").grid(column=0, row=1, padx=10, pady=5)
entry_equipe_missao = ttk.Entry(tab8)
entry_equipe_missao.grid(column=1, row=1, padx=10, pady=5)

ttk.Label(tab8, text="Data de Início (YYYY-MM-DD):").grid(column=0, row=2, padx=10, pady=5)
entry_data_inicio_missao = ttk.Entry(tab8)
entry_data_inicio_missao.grid(column=1, row=2, padx=10, pady=5)

ttk.Label(tab8, text="Data de Término (YYYY-MM-DD):").grid(column=0, row=3, padx=10, pady=5)
entry_data_termino_missao = ttk.Entry(tab8)
entry_data_termino_missao.grid(column=1, row=3, padx=10, pady=5)

ttk.Label(tab8, text="Status:").grid(column=0, row=4, padx=10, pady=5)
entry_status_missao = ttk.Entry(tab8)
entry_status_missao.grid(column=1, row=4, padx=10, pady=5)

ttk.Button(tab8, text="Registrar em Missão", command=registrar_missao_interface).grid(column=0, row=5, columnspan=2, pady=10)
...
ttk.Label(tab3, text="Membros da Equipe (separados por vírgula):").grid(column=0, row=2, padx=10, pady=5)
